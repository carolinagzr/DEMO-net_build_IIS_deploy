...
pipeline {
			...
			stages {
				stage('Source'){
					steps{
						checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'Bordados12', url: 'https://github.com/carolinagzr/DEMO-net_build_IIS_deploy.git']]])
					}
				}
				stage('Build') {
    					steps {
